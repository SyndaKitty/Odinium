...
function odinSetupDefaultImports(wasmMemoryInterface, consoleElement) {
	...
	odiniumAudio = new OdiniumAudio(wasmMemoryInterface);
		...
		"odinium": {
			...
			load_sound: (str_ptr, str_len) => {
				...
				request.onload = function() {
					odiniumAudio.audioContext.decodeAudioData(request.response, 
						function(buffer) {
							odiniumAudio.buffers.push(buffer);
							if (wasmMemoryInterface.exports.load_sound_success) {
								const odin_ctx = wasmMemoryInterface.exports.default_context_ptr();
								wasmMemoryInterface.exports.load_sound_success(id, odin_ctx);
							}
						},
						function() {
							if (wasmMemoryInterface.exports.load_sound_error) {
								const odin_ctx = wasmMemoryInterface.exports.default_context_ptr();
								wasmMemoryInterface.exports.load_sound_error(id, odin_ctx);
							}
						});
				}
				request.send();
				// TODO: will probably cause issues when loading many sounds at once
				return id;
			},
			play_sound: (id) => {
				if (id < odiniumAudio.buffers.length) {
